...
function ProjectRow({ project, isOpen, onToggle, onImageClick }) {
  const reveal = useReveal();
  return <article ref={reveal.ref} className={`${reveal.className} project-row ${isOpen ? "is-open" : ""}`}>
    <button onClick={onToggle} aria-expanded={isOpen}><span className="project-name">{project.title}</span><span className="project-category">{project.category}</span><span className="project-toggle" aria-hidden="true">{isOpen ? "−" : "+"}</span></button>
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="hero" id="top"><div ref={heroReveal.ref} className={`${heroReveal.className} hero-copy`}><p className="label">PORTFOLIO</p><h1>Luka<br /><span>Esseling</span></h1><p className="hero-note">Visuele identiteiten, posters en beeldbewerking. Gemaakt met aandacht voor kleur, compositie en detail.</p></div><HeroGrid /></section>
    <section className="work section" id="werk"><div ref={workIntroReveal.ref} className={`${workIntroReveal.className} section-intro`}><p className="label">GESELECTEERD WERK</p><h2>Werk</h2><p>Een selectie uit mijn opdrachten.</p></div><div className="project-list">{projects.map((project, index) => {
      const isOpen = openProject === index;
      return <ProjectRow key={project.title} project={project} isOpen={isOpen} onToggle={() => setOpenProject(isOpen ? null : index)} onImageClick={setLightbox} />;
    })}</div></section>
